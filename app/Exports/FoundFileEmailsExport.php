...
class FoundFileEmailsExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    ...
    */
    public function collection()
    {
        ...
        if($this->records=='all')
        {
            return Emails::select('first_name','last_name','domain','email','status','created_at')->where('user_file_id', $this->id)->get();
        }
        else
        {
            return Emails::select('first_name','last_name','domain','email','status','created_at')->where('user_file_id', $this->id)->where('status', 'Valid')->get();
        }
